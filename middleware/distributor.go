	...
	return func(c *gin.Context) {
		...
		if ok {
			...
			if shouldSelectChannel {
				...
				if channel == nil {
					abortWithOpenAiMessage(c, http.StatusServiceUnavailable, fmt.Sprintf("Model %s not found", modelRequest.Model), string(types.ErrorCodeModelNotFound))
					return
				}
			}
		}
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.Contains(c.Request.URL.Path, "/mj/") {
		...
		if c.Request.Method == http.MethodPost {
			contentType := c.Request.Header.Get("Content-Type")
			if strings.HasPrefix(contentType, "multipart/form-data") {
				// 处理 multipart/form-data 格式（图片/视频上传）
				form, err := common.ParseMultipartFormReusable(c)
				if err != nil {
					return nil, false, errors.New("无效的 video multipart 请求, " + err.Error())
				}
				defer form.RemoveAll()
				if form != nil {
					if values, ok := form.Value["model"]; ok && len(values) > 0 {
		...
		if modelRequest.Model == "" {
			modelRequest.Model = "gpt-4o"
		}
		c.Set("relay_mode", relayconstant.RelayModeFiles)
