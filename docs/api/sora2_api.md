...
Sora 2 是 OpenAI 推出的视频生成模型，支持文本生成视频（T2V）和图生视频（I2V）功能。本文档描述了如何通过 Nebula API 调用 Sora 2 进行视频生成。
...
| `data_url` | string | Data URL 格式的视频数据，可直接用于前端 `<video>` 标签或 `<img>` 标签 |
...
1. ⚠️ **配额消耗：** 视频生成会消耗大量配额，请确保账户有足够的余额
2. ⚠️ **任务保存：** 生成的任务会保留一段时间，请及时下载结果
...
- [Nebula API 主页](https://llm.ai-nebula.com)
- [视频生成接口文档](../router/视频生成接口文档.md)
